...
sub generate_downloads() {
    ...
    my $out_metrics_json = $dir_out_projects . "/$project_id" . "_metrics.json";
    print "    - Writing metrics to file [$out_metrics_json]..\n";    
    my $out_metrics = {
	'name' => 'Metrics for project' . $project_id,
	'version' => 'Generated by the dashboard',
	'children' => $project_values
    };
    open(my $fh, '>', $out_metrics_json) or die "Could not open file '$out_metrics_json' $!";
    print $fh encode_json($out_metrics);
    close $fh;

    # Write all values to single json file.
    my $out_metrics_csv = $dir_out_projects . "/$project_id" . "_metrics.csv";
    print "    - Writing metrics to file [$out_metrics_csv]..\n";    
    $out_metrics = "metric,$project_id\n";
    foreach my $line (sort keys %{$project_values}) {
	$out_metrics .= "$line," . $project_values->{$line} . "\n";
    }
    #print "DBG $out_metrics.\n\n";
    open($fh, '>', $out_metrics_csv) or die "Could not open file '$out_metrics_json' $!";
    print $fh $out_metrics;
    close $fh;
}
...
#    print Dumper(%project_values);
    ...
    &generate_downloads($project_id, $dir_out_projects, \%project_values);
